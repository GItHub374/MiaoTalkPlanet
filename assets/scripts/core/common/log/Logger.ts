...
export class Logger {
    ...
    private static log_records: string[] = [];
...
oops.log.table(object);
    ...
    static trace(msg: any, color: string = "color:#000000;") {
        ...
        let text = "%s%s"
            .replace("%s", this.getDateString())
            .replace("%s", msg);
        backLog.call(null, "%c%s", color, text);
    }
    ...
    /** 打印错误日志 */
    static logError(msg: any, describe?: string) {
        this.red(LogType.Error, msg, describe);
    }
    ...
    // 灰色
    private static red(tag: LogType, msg: any, describe?: string) {
        this.print(tag, msg, "color:red;", describe)
    }
    ...
    public static get_logs_record():string[] {
