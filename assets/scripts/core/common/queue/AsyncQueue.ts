...
export type NextQueueFunc = () => void;

/**
 * nextArgs 上一个任务传给下一个任务的参数
 */
export type AsyncCallback = (next: NextQueueFunc, params: any) => void;
...
export class AsyncQueue {
    ...
    // 任务task的唯一标识
    private static _uuid_count: number = 1;
     ...
     * 任务队列完成回调
     */
    private _complete: Function | null = null;
    public set completeFunc(func: Function){
        this._complete = func;
    }
    ...
    public push(callback: AsyncCallback, params: any = null): number {
        let uuid = AsyncQueue._uuid_count++;
        this._queues.push({
            uuid: uuid,
            callbacks: [callback],
            params: params
    ...
    protected next(taskUUID: number) {
