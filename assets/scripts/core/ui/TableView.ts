import { CCInteger, Component, Enum, EventHandler, ScrollView, Size, _decorator, error, Node, UITransform, Vec2, v2, Vec3, Prefab, assert, instantiate } from "cc";
 ...
 *          }
 *          tableCellUpdate(node : Node, idx: number, data : any) {
...
export abstract class TableViewItem extends Component {
    abstract refreshContent( idx : number, data : any ) : void;
}
...
export class TableView extends Component {
    ...
    @property({type: Node, tooltip: "裁剪节点"})
    ...
    @property({type: EventHandler, tooltip: "单元格的大小\n函数形参 (idx: number) => cc.Size"})
    private handlerCellSize: EventHandler = new EventHandler();

    @property({type: EventHandler, tooltip: "更新单元格\n函数形参 (node : Node, idx: number, data: any) => void"})
    private handlerCellUpdate: EventHandler = new EventHandler();

    @property({type: Prefab, tooltip: "单元格预制体"})
    private cellPrefab: Prefab | null = null;

    //单元格数量获取函数
    private tableCellCount(){
        return this._data.length
    }              
    //索引的单元格大小获取函数                         
    private tableCellSize: (idx: number) => Size = (idx: number) => new Size(0,0);                              
    //索引的单元格更新回调函数
    private tableCellUpdate: (node : Node, idx: number, data: any) => void = (node : Node, idx: number, data: any) => {};      
    ...
    private onInit() {
        ...
        // 获取EventHandler的回调函数
        let getEventHandlerCallback = (handler: EventHandler) => {
            let componentName = handler["_componentName"];
            let funcName = handler.handler;
            let component : any = handler.target && handler.target.getComponent(componentName);
            return component && component[funcName].bind(component);
        }
    ...
    //加载数据
    public reloadData<T>(data: T[], isUseLastOffset: boolean = false) {
        this.onInit();
        if (this.scrollView.horizontal === this.scrollView.vertical) {
            error("TableView only vertical or horizontal scrolling is supported");
            return;
        }
        this._data = data;
    ...
    public scrollToIndex(index: number, timeInSecond: number = 0.01) {
        ...
        if (this.scrollView.horizontal) {
            this.scrollView.scrollToOffset(v2(cellPos - this.midInterval, 0), Math.max(timeInSecond, 0.01));
        } else {
            if (this.vordering === VerticalFillOrder.TOP_DOWN) {
                this.scrollView.scrollToOffset(v2(0, cellPos - this.midInterval), Math.max(timeInSecond, 0.01));
            } else {
                let itemSize = this.tableCellSize(index);
                this.scrollView.scrollToOffset(v2(0, this.scrollView.getMaxScrollOffset().y - cellPos + this.midInterval), Math.max(timeInSecond, 0.01));
            }
        }
    }

    public scrollToTop(timeInSecond: number = 0.01, attenuated?: boolean): void {
        this.scrollView.scrollToTop(timeInSecond, attenuated)
    }

    public scrollToBottom(timeInSecond: number = 0.01, attenuated?: boolean): void {
    ...
    private _updateContentSize(isUseLastOffset: boolean) {
        let mask_transform = this.maskNode?.getComponent(UITransform)
        let size = mask_transform?.contentSize!
        let cellsCount = this.tableCellCount();
        if (cellsCount > 0) {
            let maxPosition = this._cellsPositions[cellsCount];
            if (this.scrollView.horizontal && maxPosition > size.width)
                size = new Size(maxPosition, size.height);
            else if (this.scrollView.vertical && maxPosition > size.height)
                size = new Size(size.width, maxPosition);
        }

        let content_transform = this.scrollView!.content!.getComponent(UITransform)
        if (size.width != content_transform?.contentSize.width || size.height != content_transform.contentSize.height) {
            content_transform!.setContentSize(size);
        }
        if (!isUseLastOffset) {
            if (this.scrollView.horizontal)
    ...
    private _positionFromIndex(index : number): Vec2 {
        let pos = this.scrollView.horizontal ? v2(this._cellsPositions[index], 0) : v2(0, this._cellsPositions[index]);
        let size = this.tableCellSize(index);

        let content_transform = this.scrollView.content!.getComponent(UITransform)
        let content_size = content_transform?.contentSize!
        ...
        if (this.scrollView.vertical) {
            if (this.vordering === VerticalFillOrder.BOTTOM_UP)
                ...
                pos.y = content_size.height - pos.y - size.height / 2;
        } else
            pos.x = pos.x + size.width / 2;

        if (this.scrollView.vertical) {
            pos.x = content_size.width * (0.5 - content_transform?.anchorX!);
            pos.y = pos.y - content_size.height * content_transform?.anchorY!;
        } else {
            pos.x = pos.x - content_size.width * content_transform?.anchorX!;
            pos.y = content_size.height * (0.5 - content_transform?.anchorY!);
        }
        return pos;
    }
    ...
    private _moveCellOutOfSight(cell: TableViewCellNode) {
        ...
        this._indices[cell.getIdx()] = false;
        cell.reset();
        cell.active = false;
    }
    ...
    private _setIndexForCell(index: number, cell: TableViewCellNode) {
        let transform = cell.getComponent(UITransform);
        transform?.setAnchorPoint(0.5, 0.5);
        let pos : Vec2 = this._positionFromIndex(index);
        cell.setPosition( new Vec3( pos.x, pos.y, 0 ));
        cell.setIdx(index);
    }
    ...
    private _scrollViewDidScroll() {
        ...
        let mask_transform = this.maskNode?.getComponent(UITransform)
        let mask_size = mask_transform?.contentSize!

        let content_transform = this.scrollView.content!.getComponent(UITransform)
        let content_size = content_transform?.contentSize!

        if (this.vordering === VerticalFillOrder.BOTTOM_UP)
            offset.y = content_size.height - offset.y - mask_size.height;

        startIdx = this._indexFromOffset(offset.clone());
        if (startIdx === CC_INVALID_INDEX)
            startIdx = countOfItems - 1;

        offset.x += mask_size.width;
        offset.y += mask_size.height;
    ...
    private _createCellAtIndex(idx:number): TableViewCellNode {
        let cell: TableViewCellNode | null = this._dequeueCell();
        if (!cell) {
            cell = new TableViewCellNode;

            let transform = cell.addComponent(UITransform);
            if (!transform) {
                transform = cell.addComponent(UITransform)!;
            }
        }
        let transform = cell.getComponent(UITransform)!;
        transform.setContentSize(this.tableCellSize(idx));

        assert( this.cellPrefab, "cellPrefab is null" )
        let item = cell.getChildByName("business_item")
        if (!item) {
            let node = instantiate(this.cellPrefab)
            node.position = new Vec3( 0, 0, 0 )
            node.parent = cell
            node.name = "business_item"
            item = node
        }
        this.tableCellUpdate(item, idx, this._data[idx]);
